...
| Q.MS1.001.[j] | <a id="GRE model" href="#GRE model"></a>Gradient echo model <button class="md-button" onclick="copyToClip(document.getElementById('GRE model').href)">test</button> | -- | *S<sub>j</sub>* | The MR signal (magnitude, phase or complex depending on context) in compartment *j*. | a.u. | -- |
